...
pub async fn internal_server_error<T>(mut stream: &mut T) -> Result<(), Box<dyn Error + Send + Sync>>
where
    T: AsyncRead + AsyncWrite + Unpin
{
    ...
    send_response(&mut stream, 500, Some(response_headers), Some(content), None, Some(Dynamic)).await
}